...
function FollowingUser() {
  ...
  const handleRemoveFollower = async (followerId) => {
    try {
      ...
      setFollowerList((prev) => prev.filter((f) => f.subscriber._id !== followerId));
    } catch (error) {
  ...
  const handleUnfollow = async (bloggerId) => {
    try {
      ...
      setFollowingList((prev) => prev.filter((f) => f.blogger._id !== bloggerId));
    } catch (error) {
      ...
      <div className="md:col-span-2">
        ...
        <div className="flex justify-center mb-6">
          ...
          <button
            ...
            className={`ml-3 px-6 py-2 text-sm font-medium rounded-lg ${
              activeTab === "following"
                ? "bg-blue-600 text-white"
                : "bg-gray-200 text-gray-700 hover:bg-gray-300"
            }`}
