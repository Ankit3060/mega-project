...
//   // New states for following functionality
...
//       } else {
//         // Remove from suggested users if now following
...
//         // Sort users by blog count (descending)
//         const sortedUsers = Object.values(userBlogCount).sort(
//           (a, b) => b.count - a.count
//         );

//         // Top voice = the user with the highest blog count (global winner)
//         const topVoiceCandidate = sortedUsers[0] || null;
//         setTopVoice(topVoiceCandidate);

//         // Suggested users = everyone except current user and top voice
//         const filteredUsers = sortedUsers.filter(
//           (u) => u._id !== user._id && u._id !== topVoiceCandidate?._id
//         );


//         // Get all user IDs to check following status
...
//         // Always check topVoice follow status separately
...
//                         className={`flex cursor-pointer items-center gap-2 transition-colors ${blog.likes.includes(user._id)
//                           ? "text-red-500"
...
//                     className="font-semibold text-gray-900 cursor-pointer">{topVoice.fullName}</p>
//                   <p className="text-sm text-gray-500">@{topVoice.userName}</p>
//                   <p className="text-xs text-gray-400">{topVoice.count} Blogs</p>
//                 </div>
...
//                       <div>
//                         <span
//                           onClick={() => navigateTo(`/user/profile/${user._id}`)}
//                           className="font-medium text-gray-900 text-sm cursor-pointer">
//                           {user.fullName}
//                         </span>
//                         <p className="text-xs text-gray-500">@{user.userName}</p>
//                         <p className="text-xs text-gray-400">{user.count} Blogs</p>
//                       </div>
//                     </div>
//                     <button
...
//   return (
//     <div className="h-screen bg-gradient-to-br from-gray-50 to-blue-50 overflow-hidden mb-[-2.5rem]">
//       <div className="flex max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 gap-6 h-full">
        
//         {/* Left side - Blog Feed */}
//         <div className="flex-1 min-w-0 h-full overflow-y-auto pr-2 scrollbar-hide">
...
//                     <button
//                       className="bg-gradient-to-r from-blue-500 to-blue-600 cursor-pointer text-white px-4 py-2 rounded-full font-medium hover:scale-105 transition-all"
//                       onClick={() => navigateTo(`/blog/read/${blog._id}`)}
//                     >
//                       Read More
//                     </button>
...
//         {/* Right side - Sidebar */}
//         <div className="hidden xl:block w-80 h-full overflow-y-auto scrollbar-hide pl-2 space-y-6">
//           {topVoice && (
//             <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-lg">
//               <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
//                 <span className="text-2xl">🏆</span>
...
//             </h3>
//             <div className="flex flex-wrap gap-2">
//               {["Technology", "Politics", "AI", "Agriculture", "Celebraties", "Sport"].map(
//                 (topic) => (
//                   <span
//                     key={topic}
//                     onClick={() => navigateTo(`/explore?category=${topic}`)}
//                     className="px-3 py-2 text-sm rounded-full bg-gradient-to-r from-gray-100 to-gray-200 hover:from-blue-100 hover:to-blue-200 text-gray-700 hover:text-blue-700 cursor-pointer transition-all"
//                   >
//                     #{topic}
//                   </span>
//                 )
...
return (
  <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
    <div className="flex max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 gap-6">
      
      {/* Left side - Blog Feed */}
      <div className="flex-1 min-w-0 overflow-y-auto scrollbar-hide pb-20">
        <div className="space-y-8">
              ...
              <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-6">
                    ...
                    {/* Save */}
                    <button className="flex items-center gap-2 text-gray-500 hover:text-yellow-900 transition-colors">
                      <BiBookmark className="w-5 h-5" />
                      <span className="text-sm hidden sm:inline">Save</span>
                    </button>
                  </div>
                  ...
                  <button
                    className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-4 py-2 rounded-full font-medium hover:scale-105 transition-transform"
                    onClick={() => navigateTo(`/blog/read/${blog._id}`)}
                  >
                    Read More
                  </button>
      ...
      {/* Right Sidebar */}
      <div className="hidden xl:block w-80 overflow-y-auto scrollbar-hide pb-20">
        <div className="space-y-6">
          {/* keep your Top Voice, Suggested Users, Trending Topics, Newsletter sections here (no change) */}
        </div>
      </div>
    </div>
  </div>
